...
import type { TraverseClosure } from './types.js'
...
export class LinkedList<T = any> {
   ...
   */
  public map<R = any>(closure: TraverseClosure<T, R>) {
    let index = 0
   ...
   * Space complexity: O(1)
   */
  public find(closure: TraverseClosure<T>) {
    let index = 0
    let node: Node<T> | undefined = this.head
   ...
   * Space complexity: O(1)
   */
  public traverse(closure: TraverseClosure<T>) {
    let index = 0
    let node: Node<T> | undefined = this.head
   ...
   * Space complexity: O(1)
   */
  public forEach(closure: TraverseClosure<T>) {
